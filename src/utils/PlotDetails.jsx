...
const PlotDetails = () => {
  ...
  const [bookingDetails, setBookingDetails] = useState({
    ...
    email: '',
    phone: '',
    date: '', // Add date field for booking
  });
  const [bookingConfirmed, setBookingConfirmed] = useState(false);

  useEffect(() => {
    axios.get(`http://localhost:3005/views/${id}`)
      .then(response => {
        setProduct(response.data);
      })
      .catch(err => console.log(err));
  ...
  const handleSubmit = (e) => {
    ...
    const bookingData = {
      ...bookingDetails,
      productId: product.pname
    };

    axios.post('http://localhost:3005/bookingss', bookingData)
      .then(response => {
        console.log('Booking saved successfully:', response.data);
        setBookingConfirmed(true);
        setTimeout(() => {
    ...
    setBookingDetails({
      ...
      email: '',
      phone: '',
      date: '', // Reset date after submission
    });
  };
      ...
      {product && (
        <Card sx={{ maxWidth: 600, margin: 'auto', marginTop: 1 }}>
          <CardContent>
            ...
            <img
              src={`data:image/jpeg;base64,${Buffer.from(product.image2.data).toString('base64')}`}
              alt={product.name}
              style={{ maxWidth: '100%', height: 'auto', marginBottom: 15 }}
            />
            <Typography variant="body1">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <TextField
                label="Date"
                name="date"
                type="date" // Use date type input for selecting date
